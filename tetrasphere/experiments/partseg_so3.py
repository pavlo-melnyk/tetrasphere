...
def test(args, io, logger):
    ...
    test_label_seg = np.concatenate(test_label_seg)
    test_ious = calculate_shape_IoU(test_pred_seg, test_true_seg, test_label_seg, args.class_choice)
    outstr = f'Test ({args.test_rot}) :: test acc: %.6f, test avg acc: %.6f, test iou: %.6f' % (
...
if __name__ == "__main__":
    ...
    name = f"partseg_K{num_spheres}_{train_rot}_seed{seed}"
    ex = f"--exp_name {name} --epochs {num_epochs} --model {model} --num_spheres {num_spheres} --train_rot {train_rot}"
    experiments.append(ex)

    print(f"array_size={len(experiments)}, {num_epochs=}, {epochs_per_hour=}")

    array_id = int(os.environ.get('SLURM_ARRAY_TASK_ID', "-1"))
    if array_id == -1:
        for args in experiments:
            # args = args + " --batch_size 8 --test_batch_size 8"
            main(args.split())
    else:
